...
@[blueprint
  ...
  (latexEnv := "sublemma")]
theorem log_le (t : ℝ) (ht : t > -1) :
    log (1 + t) ≤ t :=
  (Real.log_le_sub_one_of_pos
    (neg_lt_iff_pos_add'.mp ht)).trans add_tsub_le_left
...
theorem log_ge {t : ℝ} (ht : 0 ≤ t) :
  ...
  have hf_deriv_pos : ∀ s > 0, 0 ≤ deriv f s := by
    ...
    ring_nf
    nlinarith [inv_mul_cancel₀
      (by positivity : (1 + s) ≠ 0)]
  have h_mvt :
      ∃ c ∈ Set.Ioo 0 t,
        deriv f c = (f t - f 0) / (t - 0) := by
  ...
  nlinarith [hf_deriv_pos c hc₁, mul_div_cancel₀
    (log (1 + t) - (t - t ^ 2 / 2)) (by positivity)]
...
@[blueprint
  ...
  (latexEnv := "sublemma")
  (discussion := 766)]
theorem log_ge' {t t₀ : ℝ} (ht : 0 ≤ t) (ht0 : t ≤ t₀)
    (ht0' : t₀ < 1) :
  ...
  · field_simp [ht.ne]
    rfl
  have := strictConcaveOn_log_Ioi.2
    (y := 1) (x := 1 - t₀) (by grind) (by grind) (by linarith)
  simp only [smul_eq_mul, log_one, mul_zero,
    add_zero, mul_one] at this
  convert this (a := t / t₀) (b := 1 - t / t₀)
    (by bound) (by bound) (by ring) |>.le using 2
...
theorem symm_inv_log
  ...
  have log_add_ne_zero : log (1 + t) ≠ 0 := by
    simp only [ne_eq, log_eq_zero]; grind
  ...
  have denominator1 :=
    le_neg.mpr <| log_le (-t) (by linarith)
  ...
  _ = (-(log (1 - t ^ 2))) /
    ...
    rw [abs_div, abs_mul,
      abs_of_nonpos <| log_nonpos (by bound)
        (by simp only [tsub_le_iff_right,
          le_add_iff_nonneg_right]; positivity),
      abs_of_nonneg <| log_nonneg (by linarith),
      abs_of_nonpos <|
        log_nonpos (by linarith) (by linarith)]
  _ ≤ (-t ^ 2 / (1 / 4) * log (3 / 4)) /
        (log (1 + t) * -log (1 - t)) := by
    gcongr
    · apply mul_nonneg
...
theorem li.sub_Li
    ...
    li x - Li x = li 2 := by
  have : Filter.Tendsto
      (fun ε => ∫ t in Set.Ioc 0 x \
        Set.Ioo (1 - ε) (1 + ε), 1 / log t)
      (𝓝[>] 0)
      (nhds (Li2Bounds.li2_symmetric + Li x)) := by
    apply Filter.Tendsto.congr'
      (f₁ := fun ε ↦
        (∫ t in (0:ℝ)..(1 - ε), 1 / log t) +
        (∫ t in (1 + ε)..2, 1 / log t) + Li x)
    · filter_upwards [Ioo_mem_nhdsGT
        (by linarith : (0 : ℝ) < 1)] with ε hε
      rw [Li2Bounds.setDiff_integral_eq_split
        ε x hε.1 hε.2 h2x]
      unfold Li
      rw [add_assoc,
    ...
    · apply Filter.Tendsto.add_const
      ...
      filter_upwards [Ioo_mem_nhdsGT
        (by linarith : (0 : ℝ) < 1)] with ε hε using
        Li2Bounds.setDiff_integral_eq_split
          ε 2 hε.1 hε.2 (by rfl)
...
theorem li_eq_Li2Bounds_li (x : ℝ) :
    li x = Li2Bounds.li x := rfl
...
@[blueprint
  ...
  (latexEnv := "sublemma")
  (discussion := 759)]
theorem li.two_approx_weak :
    li 2 ∈ Set.Icc 1.039 1.06 := by
...
theorem li2_symmetric_eq_li2 :
    Li2Bounds.li2_symmetric = li 2 := by
  rw [li_eq_Li2Bounds_li]
