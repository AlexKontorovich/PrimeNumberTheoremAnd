...
theorem lemma_abadsumas {s : ℂ} (hs1 : s ≠ 1) (hsigma : 0 ≤ s.re) {a b : ℝ} (ha : 0 < a)
    ...
    let C : ℝ :=
      if ϑ ≠ 0 then
        ...
        s.re / 6
    Summable (fun n : ℕ ↦ FourierTransform.fourier f (n + 1) + FourierTransform.fourier f (-(n + 1))) ∧
    ∃ E : ℂ, ∑' n : ℕ, (FourierTransform.fourier f (n + 1) + FourierTransform.fourier f (-(n + 1))) =
  ...
  have error_term : ℕ → ℂ := fun n ↦ Classical.choose (apply_applem n)

  have error_term_eq : ∀ n : ℕ,
    FourierTransform.fourier f (n + 1) + FourierTransform.fourier f (-(n + 1 : ℤ)) =
    (a ^ (-s) / (2 * π * I)) * ((-1) ^ (n + 2) * 2 * ϑ / ((n + 1) ^ 2 - ϑ ^ 2)) -
    (b ^ (-s) / (2 * π * I)) * ((-1) ^ (n + 2) * 2 * ϑ_minus / ((n + 1) ^ 2 - ϑ_minus ^ 2)) +
    error_term n := by
  ...
  have error_term_bound : ∀ n : ℕ, ‖error_term n‖ ≤
    (a ^ (-(s.re + 1))) / (4 * π ^ 2) *
    (s.re / ((n + 1) - ϑ) ^ 2 + s.re / ((n + 1) + ϑ) ^ 2 +
     |ϑ| / |(n + 1) - ϑ| ^ 3 + |ϑ| / |(n + 1) + ϑ| ^ 3) := by
    intro n
  ...
  have sum_main_term_b : ∑' n : ℕ,
    ((-1 : ℂ) ^ (n + 2) * 2 * ϑ_minus / ((n + 1) ^ 2 - ϑ_minus ^ 2)) = π * g ϑ_minus := by
    sorry
  ...
  have total_error_bound : ‖∑' n : ℕ, error_term n‖ ≤ C / a ^ (s.re + 1) := by
    sorry

  have main_summable_a : Summable (fun n : ℕ ↦
    ((-1) ^ (n + 2) * 2 * ↑ϑ / ((↑n + 1) ^ 2 - ↑ϑ ^ 2) : ℂ)) := by
    -- First establish |ϑ| < 1 from the hypothesis haτ : a > |s.im| / (2 * π)
    have hϑ_lt_one : |ϑ| < 1 := by
      have h2pa : 0 < 2 * π * a := by
        have := Real.pi_pos
        positivity
      have hbound : |s.im| < 2 * π * a := by
        have h2p : 0 < 2 * π := by
          have := Real.pi_pos
          positivity
        replace haτ := (div_lt_iff₀ h2p).mp haτ
        linarith
      simp only [ϑ]
      rw [abs_div, abs_of_pos h2pa, div_lt_one h2pa]
      exact hbound
    -- Therefore the denominator (n+1)^2 - ϑ^2 is positive for all n
    have hdenom_pos : ∀ n : ℕ, (0 : ℝ) < (n + 1) ^ 2 - ϑ ^ 2 := by
      intro n
      have : ϑ ^ 2 < 1 := by nlinarith [abs_nonneg ϑ, sq_abs ϑ]
      have : (0 : ℝ) < (n + 1) ^ 2 := by positivity
      nlinarith [Nat.cast_nonneg n, sq_nonneg (n : ℝ)]

    -- Bound the norm by 2|ϑ| / ((n+1)^2 - ϑ^2) ≤ 2|ϑ| / (n+1)^2 / (1 - ϑ^2)
    apply Summable.of_norm_bounded (g := fun n : ℕ => 2 * |ϑ| / ((1 - ϑ ^ 2) * (↑n + 1) ^ 2))
    · -- The bounding series is summable: 2|ϑ| * ∑ 1/(n+1)^2 < ∞
      apply Summable.mul_left
      -- Factor the constant out
      have hpos : (0 : ℝ) < 1 - ϑ ^ 2 := by
        have := hϑ_lt_one
        nlinarith [sq_abs ϑ, abs_nonneg ϑ]
      -- Rewrite as product of constant and (n+1)^{-2}
      simp_rw [mul_inv]
      apply Summable.mul_left

      have hbase : Summable (fun n : ℕ => ((n : ℕ) ^ 2 : ℝ)⁻¹) := by
        simp_rw [inv_eq_one_div]
        norm_cast
        simp_rw [Nat.cast_pow] at ⊢
        apply Real.summable_one_div_nat_pow.mpr (by norm_num)
      -- Step 2: shift by 1 to get ∑ 1/(n+1)²
      exact (summable_nat_add_iff 1).mpr hbase |>.congr
        (fun n => by push_cast; ring_nf)

    · -- Each term is norm-bounded
      intro n
      -- Subgoal 1: ϑ² < 1, needed to show positivity of the RHS denominator
      have hϑ_sq_lt : ϑ ^ 2 < 1 := by
        nlinarith [sq_abs ϑ, abs_nonneg ϑ]

      -- Subgoal 2: the norm on the LHS collapses to a real expression
      have hnorm_eq : ‖(-1 : ℂ) ^ (n + 2) * 2 * ↑ϑ / ((↑n + 1) ^ 2 - ↑ϑ ^ 2)‖ =
          2 * |ϑ| / ((↑n + 1) ^ 2 - ϑ ^ 2) := by
        have hdp := hdenom_pos n
        -- Cast the denominator to a single real embedding
        have hdenom_eq : ((↑n : ℂ) + 1) ^ 2 - (↑ϑ : ℂ) ^ 2 =
            ↑(((n : ℂ) + 1) ^ 2 - ϑ ^ 2) := by push_cast; ring
        -- (-1)^(n+2) has norm 1
        have hminus1 : ‖(-1 : ℂ) ^ (n + 2)‖ = 1 := by
          rw [norm_pow, norm_neg, norm_one, one_pow]
        -- Unfold the norm of a quotient/product
        simp;
        have hpos : (↑n + 1) ^ 2 - ϑ ^ 2 ≠ 0 := ne_of_gt hdp
        field_simp [hpos]
        have : ((↑n + 1) ^ 2 - ϑ ^ 2) / ‖((↑n + 1) ^ 2 - (↑ϑ : ℂ) ^ 2)‖ = 1 := by
          have hcast : ‖((↑n + 1) ^ 2 - (↑ϑ : ℂ) ^ 2)‖ = (↑n + 1) ^ 2 - ϑ ^ 2 := by
            have : ((↑n + 1) ^ 2 - (↑ϑ : ℂ) ^ 2) = ↑((n + 1 : ℂ) ^ 2 - ϑ ^ 2) := by push_cast; ring
            rw [this]
            rw [show (↑n + 1) ^ 2 - (↑ϑ : ℂ) ^ 2 = ((↑n + 1) ^ 2 - ϑ ^ 2 : ℝ) by push_cast; ring]
            rw [Complex.norm_real]
            exact Real.norm_of_nonneg (by linarith)
          rw [hcast, div_self (ne_of_gt hdp)]
        rw [mul_div_assoc, this]
        simp

      -- Subgoal 2: the norm on the LHS collapses to a real expression
      have hdenom_ineq : (1 - ϑ ^ 2) * ((↑n : ℝ) + 1) ^ 2 ≤ (↑n + 1) ^ 2 - ϑ ^ 2 := by
        have hn_cast : (0 : ℝ) ≤ (n : ℝ) := Nat.cast_nonneg n
        have h1 : (1 : ℝ) ≤ ((n : ℝ) + 1) ^ 2 := by nlinarith
        have h2 : 0 ≤ ϑ ^ 2 * (((n : ℝ) + 1) ^ 2 - 1) :=
          mul_nonneg (sq_nonneg ϑ) (by linarith)
        nlinarith [sq_nonneg ϑ]

      -- Subgoal 3: the algebraic denominator comparison
      -- (1 - ϑ²)(n+1)² ≤ (n+1)² - ϑ²  ⟺  ϑ² ≤ ϑ²(n+1)²  ⟺  1 ≤ (n+1)²
      have hdenom_ineq : (1 - ϑ ^ 2) * ((↑n : ℝ) + 1) ^ 2 ≤ (↑n + 1) ^ 2 - ϑ ^ 2 := by
        nlinarith [sq_nonneg ϑ, hdenom_pos n, sq_nonneg ((↑n : ℝ) + 1)]

      -- Conclude: rewrite with the norm identity, then use denominator monotonicity
      rw [hnorm_eq]
      apply div_le_div_of_nonneg_left
        (by positivity)
        (by nlinarith [hdenom_pos n, sq_nonneg ϑ])
        hdenom_ineq

      -- nlinarith [sq_nonneg (n : ℝ), sq_nonneg ϑ]
  ...
  have summable_term_a : Summable (fun n : ℕ ↦
    ...
    exact main_summable_a.mul_left ((a : ℂ) ^ (-s) / (2 * (π : ℂ) * I))
  ...
  have summable_term_b : Summable (fun n : ℕ ↦
    ...
    exact main_summable_b.mul_left ((b : ℂ) ^ (-s) / (2 * (π : ℂ) * I))
  ...
  · -- Prove summability of Fourier terms
    ...
    have fourier_decomp : (fun n : ℕ ↦ FourierTransform.fourier f (n + 1) +
      FourierTransform.fourier f (-↑((n : ℤ) + 1))) =
  ...
  · -- Prove the main equality with error bound
    ...
    · -- Prove equality
      ...
      calc ∑' (n : ℕ), (FourierTransform.fourier f (↑n + 1) + FourierTransform.fourier f (-↑((n : ℤ) + 1)))
        _ = ∑' (n : ℕ), (↑a ^ (-s) / (2 * ↑π * I) * ((-1) ^ (n + 2) * 2 * ↑ϑ / ((↑n + 1) ^ 2 - ↑ϑ ^ 2)) -
            ↑b ^ (-s) / (2 * ↑π * I) * ((-1) ^ (n + 2) * 2 * ↑ϑ_minus / ((↑n + 1) ^ 2 - ↑ϑ_minus ^ 2)) +
            error_term n) := by
          congr
          ext n; apply error_term_eq n
        _ = ∑' (n : ℕ), (↑a ^ (-s) / (2 * ↑π * I) * ((-1) ^ (n + 2) * 2 * ↑ϑ / ((↑n + 1) ^ 2 - ↑ϑ ^ 2)) -
            ↑b ^ (-s) / (2 * ↑π * I) * ((-1) ^ (n + 2) * 2 * ↑ϑ_minus / ((↑n + 1) ^ 2 - ↑ϑ_minus ^ 2))) +
            ∑' (n : ℕ), error_term n := by
          rw [Summable.tsum_add]
        ...
        _ = ∑' (n : ℕ), ↑a ^ (-s) / (2 * ↑π * I) * ((-1) ^ (n + 2) * 2 * ↑ϑ / ((↑n + 1) ^ 2 - ↑ϑ ^ 2)) -
            ∑' (n : ℕ), ↑b ^ (-s) / (2 * ↑π * I) * ((-1) ^ (n + 2) * 2 * ↑ϑ_minus / ((↑n + 1) ^ 2 - ↑ϑ_minus ^ 2)) +
            ∑' (n : ℕ), error_term n := by
          rw [Summable.tsum_sub summable_term_a summable_term_b]
        _ = ↑a ^ (-s) / (2 * ↑π * I) * ∑' (n : ℕ), ((-1) ^ (n + 2) * 2 * ↑ϑ / ((↑n + 1) ^ 2 - ↑ϑ ^ 2)) -
            ↑b ^ (-s) / (2 * ↑π * I) * ∑' (n : ℕ), ((-1) ^ (n + 2) * 2 * ↑ϑ_minus / ((↑n + 1) ^ 2 - ↑ϑ_minus ^ 2)) +
            ∑' (n : ℕ), error_term n := by
        ...
        have htsum_reindex : ∑' (n : ℕ),
          ...
          · rw [Summable.tsum_eq_zero_add]
            ...
            have split_sum : ∑' (k : ℕ), (FourierTransform.fourier f (↑k + 1) + FourierTransform.fourier f (-(↑k + 1 : ℤ))) =
              ...
              sorry  -- This uses tsum_eq_zero_add and series manipulation
