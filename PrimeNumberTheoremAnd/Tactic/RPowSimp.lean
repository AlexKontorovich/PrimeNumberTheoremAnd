...
theorem pow_pos (ha : 0 < a) (hb : 0 < b) {e : ℝ} : 0 < (a ^ e * b : ℝ) :=
  mul_pos (rpow_pos_of_pos ha e) hb

theorem pow_pf (ha : 0 < a) (hb : 0 < b) {e₁ e₂ : ℝ} (_ : b ^ e₂ = b') :
    (a ^ e₁ * b : ℝ) ^ e₂ = a ^ (e₁ * e₂) * b' := by
  subst_vars; rw [mul_rpow (rpow_pos_of_pos ha e₁).le hb.le, rpow_mul ha.le]

def evalPow (va : ExProd a) (e : Q(ℝ)) : Option (Q(0 < $a) × Result ExProd q(Real.rpow $a $e)) :=
  match va with
...
theorem pow_congr {b : ℝ} (_ : a = a') (_ : a' ^ b = c) : (a ^ b : ℝ) = c := by subst_vars; rfl

theorem inv_congr (_ : a = a') (_ : a' ^ (-1 : ℝ) = b) : (a⁻¹ : ℝ) = b := by
  subst_vars; simp [rpow_neg_one]

theorem npow_congr {b : ℕ} {c : ℝ} (_ : a = a') (_ : a' ^ (b : ℝ) = c) : Monoid.npow b a = c := by
