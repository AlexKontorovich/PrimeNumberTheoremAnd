...
theorem log_one_minus_integrable (ε : ℝ) (hε : 0 < ε)
    (hε1 : ε < 1) :
    IntervalIntegrable (fun u => 1 / log (1 - u))
      volume ε 1 := by
  rw [intervalIntegrable_iff_integrableOn_Ioc_of_le hε1.le]
  refine Measure.integrableOn_of_bounded (M := 1 / ε)
    measure_Ioc_lt_top.ne
    (Measurable.aestronglyMeasurable (by fun_prop)) ?_
  filter_upwards [self_mem_ae_restrict (by measurability),
    Measure.ae_ne _ 1] with u ⟨hε_lt_u, hu_le_one⟩ hu_ne_one
...
/-- 1/log(1+u) is integrable on [ε, 1] for ε > 0. -/
theorem log_one_plus_integrable (ε : ℝ) (hε : 0 < ε)
    (hε1 : ε < 1) :
    IntervalIntegrable (fun u => 1 / log (1 + u))
      volume ε 1 := by
  refine ContinuousOn.intervalIntegrable_of_Icc hε1.le
    fun u hu ↦ ContinuousAt.continuousWithinAt ?_
  have : 1 + u ≠ 0 := by grind
...
theorem g_intervalIntegrable_full :
  ...
  · filter_upwards [self_mem_ae_restrict
      (by measurability)] with t ht
    have habs : |g t| = g t :=
      abs_of_pos <| Li2.g_pos t ht.1 ht.2
    simpa [Real.norm_eq_abs, habs] using
      Li2.g_le_two t ht.1 ht.2

/-- g is integrable on any subinterval [a, b] ⊆ (0, 1). -/
theorem g_intervalIntegrable (a b : ℝ) (ha : 0 < a)
    (hb : b < 1) (hab : a ≤ b) :
...
@[blueprint
  ...
  (discussion := 759)]
theorem li2_symmetric_lower :
    (1039 : ℚ) / 1000 ≤ li2_symmetric := by
...
@[blueprint
  ...
  (discussion := 759)]
theorem li2_symmetric_upper :
    li2_symmetric ≤ (106 : ℚ) / 100 := by
  rw [li2_symmetric_eq_Li2_li2]
  exact Li2.li2_upper
...
theorem li2_symmetric_bounds :
    (1039 : ℚ) / 1000 ≤ li2_symmetric ∧
      li2_symmetric ≤ (106 : ℚ) / 100 :=
...
theorem integral_sub_right (ε : ℝ) (_hε : 0 < ε)
  ...
  have h := intervalIntegral.integral_comp_add_right
    (fun x => 1 / log x) 1 (a := ε) (b := 1)
  simp only [show ε + (1 : ℝ) = 1 + ε from by ring,
    show (1 : ℝ) + 1 = 2 from by ring] at h
  simpa [show ∀ u : ℝ, 1 / log (u + 1) = 1 / log (1 + u)
    from fun u ↦ by ring_nf] using h.symm

/-- The principal value integral for li(2) equals ∫_ε^1 g(u) du. -/
theorem pv_integral_eq_symmetric (ε : ℝ) (hε : 0 < ε)
    (hε1 : ε < 1) :
    (∫ t in (0 : ℝ)..(1 - ε), 1 / log t) +
      ∫ t in (1 + ε)..(2 : ℝ), 1 / log t =
      ∫ u in ε..1, g u := by
  rw [integral_sub_left ε hε hε1,
    integral_sub_right ε hε hε1,
    ← intervalIntegral.integral_add
      (log_one_minus_integrable ε hε hε1)
...
theorem limit_integral_g :
    Filter.Tendsto (fun ε => ∫ u in ε..1, g u)
      (nhdsWithin 0 (Ioi 0))
      (nhds (∫ u in (0 : ℝ)..1, g u)) := by
  have heq' : (fun ε => ∫ u in ε..(1 : ℝ), g u) =
      (fun ε => -∫ u in (1 : ℝ)..ε, g u) :=
    funext fun ε => intervalIntegral.integral_symm 1 ε
  have hval : -∫ u in (1 : ℝ)..0, g u =
      ∫ u in (0 : ℝ)..1, g u := by
    rw [intervalIntegral.integral_symm 0 1]; ring
  rw [heq', ← hval]
  have hcont : ContinuousOn
      (fun x => ∫ u in (1 : ℝ)..x, g u) (Icc 0 1) := by
    rw [← Set.uIcc_of_le (by norm_num : (0 : ℝ) ≤ 1)]
    exact intervalIntegral.continuousOn_primitive_interval'
      g_intervalIntegrable_full (by simp)
  exact ((hcont.neg 0
    (Set.left_mem_Icc.2 (by norm_num))).mono_left
    (nhdsWithin_mono 0
      Ioo_subset_Icc_self)).mono_left
    (le_of_eq (nhdsWithin_Ioo_eq_nhdsGT
...
theorem setDiff_decompose (ε x : ℝ) (hε : 0 < ε)
    ...
    Set.Ioc 0 x \ Set.Ioo (1 - ε) (1 + ε) =
      Set.Ioc 0 (1 - ε) ∪ Set.Icc (1 + ε) x := by
  grind

/-- The Set.diff integral equals the split interval integrals. -/
theorem setDiff_integral_eq_split (ε x : ℝ) (hε : 0 < ε)
    (hε1 : ε < 1) (hx : 2 ≤ x) :
    ∫ t in Set.Ioc 0 x \ Set.Ioo (1 - ε) (1 + ε),
      1 / log t =
    (∫ t in (0 : ℝ)..(1 - ε), 1 / log t) +
      ∫ t in (1 + ε)..x, 1 / log t := by
  rw [setDiff_decompose ε x hε hx,
    setIntegral_union (by grind) measurableSet_Icc,
    intervalIntegral.integral_of_le (by linarith),
    integral_Icc_eq_integral_Ioc,
    intervalIntegral.integral_of_le (by linarith)]
  · have hlog_neg : log (1 - ε) < 0 :=
      Real.log_neg (by linarith) (by linarith)
    have hcont :
        ContinuousOn (fun t => 1 / log t)
          (Set.Ioc 0 (1 - ε)) :=
      ContinuousOn.div continuousOn_const
        (Real.continuousOn_log.mono
          fun x hx => ne_of_gt hx.1)
        fun x hx =>
          Real.log_ne_zero_of_pos_of_ne_one hx.1
            (by linarith [hx.2])
...
@[blueprint
  ...
  (discussion := 764)]
theorem li2_symmetric_eq_li2 :
    li2_symmetric = li 2 :=
  (pv_tendsto_li2_symmetric.limUnder_eq).symm

/-- The main result: certified bounds on li(2). -/
theorem li2_bounds :
    (1039 : ℚ) / 1000 ≤ li 2 ∧ li 2 ≤ (106 : ℚ) / 100 := by
  rw [← li2_symmetric_eq_li2]
  exact li2_symmetric_bounds

/-- Proof of li.two_approx_weak. -/
theorem li_two_approx_weak_proof :
    li 2 ∈ Set.Icc 1.039 1.06 :=
  ⟨by have := li2_bounds.1
      simp only [Rat.cast_ofNat] at this
      linarith,
   by have := li2_bounds.2
      simp only [Rat.cast_ofNat] at this
      linarith⟩
