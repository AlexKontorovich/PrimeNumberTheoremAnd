...
@[blueprint
  ...
  (statement := /-- $\rho$ is understood to lie in the set $\{s: \zeta(s)=0\}$, counted with multiplicity. We will often restrict the zeroes $\rho$ to a rectangle $\{ \Re \rho \in I, \Im \rho \in J \}$, for instance through sums of the form $\sum_{\Re \rho \in  I, \Im \rho \in J} f(\rho).$. -/)]
...
@[blueprint
  "RH-up-to"
  (statement := /-- We say that the Riemann hypothesis has been verified up to height $T$ if there are no zeroes in the rectangle $\{ \Re \rho \in (0.5, 1), \Im \rho \in [0,T] \}$. -/)]
noncomputable def riemannZeta.RH_up_to (T : ℝ) : Prop := IsEmpty (riemannZeta.zeroes_rect (Set.Ioo 0.5 1) (Set.Icc 0 T))
