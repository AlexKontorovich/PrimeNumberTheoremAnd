...
private theorem tsum_succ_geom (x : ℂ) (h : ‖x‖ < 1) :
  ...
  simpa [Nat.choose_one_right, one_div, inv_pow] using
    tsum_choose_mul_geometric_of_norm_lt_one (𝕜 := ℂ) 1 h

/-- Summability of `n ↦ (n+1) x^n` for `‖x‖ < 1`. -/
private theorem summable_succ_geom (x : ℂ) (h : ‖x‖ < 1) : Summable (fun n : ℕ => ((n : ℂ) + 1) * x ^ n) := by
  simpa [Nat.choose_one_right] using summable_choose_mul_geometric_of_norm_lt_one (R := ℂ) 1 h
...
private theorem ram_eulerProduct_hasProd (α β s : ℂ) (h1 : 1 < s.re) (h2 : 1 < (s - α).re)
    (h3 : 1 < (s - β).re) (h4 : 1 < (s - α - β).re) :
...
/-- B4 (tprod form). -/
private theorem ram_eulerProduct_tprod (α β s : ℂ) (h1 : 1 < s.re) (h2 : 1 < (s - α).re)
    (h3 : 1 < (s - β).re) (h4 : 1 < (s - α - β).re) :
