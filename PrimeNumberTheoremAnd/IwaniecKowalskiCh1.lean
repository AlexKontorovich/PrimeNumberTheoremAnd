...
lemma unique_divisor_decomposition {a b d : ℕ} (hab : Coprime a b) (hd : d ∣ a * b) :
  ...
  · exact?;
  · rintro ⟨ x, y ⟩ ⟨ hx, hy, h ⟩;
    refine' Prod.ext _ _ <;> subst h <;> simp_all +decide [ Nat.gcd_eq_left_iff_dvd, Nat.Coprime.gcd_mul ];
    · refine' Eq.symm ( Nat.dvd_antisymm _ _ );
      · exact Nat.Coprime.dvd_of_dvd_mul_right ( show Nat.Coprime ( Nat.gcd ( x * y ) a ) y from Nat.Coprime.coprime_dvd_left ( Nat.gcd_dvd_right _ _ ) <| Nat.Coprime.coprime_dvd_right hy <| by aesop ) <| Nat.gcd_dvd_left _ _;
...
theorem sum_divisors_mul_of_coprime {R : Type*} [CommRing R]
  ...
  have h_divisors : Nat.divisors (a * b) = Finset.image (fun (p : ℕ × ℕ) => p.1 * p.2) (Nat.divisors a ×ˢ Nat.divisors b) := by
    exact Nat.divisors_mul _ _;
  rw [ h_divisors, Finset.sum_image, Finset.sum_product ];
  · rw [ Finset.sum_mul ];
    exact Finset.sum_congr rfl fun i hi => by rw [ Finset.mul_sum _ _ _ ] ; exact Finset.sum_congr rfl fun j hj => hf.map_mul_of_coprime ( Nat.Coprime.gcd_eq_one <| hab.coprime_dvd_left ( Nat.dvd_of_mem_divisors hi ) |> Nat.Coprime.coprime_dvd_right ( Nat.dvd_of_mem_divisors hj ) ) ;
  · intro p hp q hq h_eq; simp_all +decide [ mul_eq_mul_left_iff, Nat.mem_divisors ] ;
    have h1 : p.1 = q.1 := by
      exact Nat.dvd_antisymm ( by exact Nat.Coprime.dvd_of_dvd_mul_right ( Nat.Coprime.coprime_dvd_left hp.1 <| Nat.Coprime.coprime_dvd_right hq.2 hab ) <| h_eq ▸ dvd_mul_right _ _ ) ( by exact Nat.Coprime.dvd_of_dvd_mul_right ( Nat.Coprime.coprime_dvd_left hq.1 <| Nat.Coprime.coprime_dvd_right hp.2 hab ) <| h_eq.symm ▸ dvd_mul_right _ _ );
    aesop
...
theorem LSeries_d_summable (k : ℕ) {s : ℂ} (hs : 1 < s.re) : LSeriesSummable (↗(d k)) s := by
  have h_d_k_mul : ∀ k : ℕ, (d (k + 1) : ArithmeticFunction ℂ) = (d k : ArithmeticFunction ℂ) * zeta := by
    intro k; exact (by
  ...
  · refine' ⟨ _, hasSum_single 1 _ ⟩;
    ...
    unfold d; aesop;
  · have h_d_k_plus_1 : (d (k + 1) : ArithmeticFunction ℂ) = (d k : ArithmeticFunction ℂ) * zeta := by
      exact h_d_k_mul k;
...
theorem d_apply_prime_pow {k : ℕ} (hk : 0 < k) {p : ℕ} (hp : p.Prime) (a : ℕ) :
  ...
  · have h_hockey_stick : ∑ j ∈ Finset.range (a + 1), Nat.choose (j + k - 1) (k - 1) = Nat.choose (a + k) k := by
      exact Nat.recOn a ( by simp +arith +decide ) fun n ih => by cases k <;> simp_all +decide [ Nat.choose, add_comm, add_left_comm, Finset.sum_range_succ ] ;
    rw [ ← h_hockey_stick, Nat.sum_divisorsAntidiagonal fun x y => if y = 0 then 0 else ( d k ) x ];
    rw [ Nat.sum_divisors_prime_pow hp ];
    exact Finset.sum_congr rfl fun x hx => by rw [ if_neg ( Nat.ne_of_gt ( Nat.div_pos ( pow_le_pow_right₀ hp.pos ( Finset.mem_range_succ_iff.mp hx ) ) ( pow_pos hp.pos _ ) ) ), ih ] ;
...
theorem LSeries_sigma_eq_riemannZeta_mul (ν : ℂ) {s : ℂ} (hs : 1 < s.re) (hsν : 1 < (s - ν).re) :
  ...
  have h_lseries_def : ∀ (f : ArithmeticFunction ℂ) (s : ℂ), 1 < s.re → ∑' n : ℕ+, LSeries.term f s n = (∑' n : ℕ+, f n * (n : ℂ) ^ (-s)) := by
    simp +decide [ LSeries.term ];
    exact fun f s hs => tsum_congr fun n => by rw [ div_eq_mul_inv, Complex.cpow_neg ] ;
  have h_fubini : ∑' (n : ℕ+), (∑ d ∈ (n : ℕ).divisors, (d : ℂ) ^ ν) * (n : ℂ) ^ (-s) = ∑' (d : ℕ+), (d : ℂ) ^ ν * ∑' (m : ℕ+), (d * m : ℂ) ^ (-s) := by
    have h_fubini : Summable (fun p : ℕ+ × ℕ+ => (p.1 : ℂ) ^ ν * (p.1 * p.2 : ℂ) ^ (-s)) := by
      have h_fubini : Summable (fun p : ℕ+ × ℕ+ => (p.1 : ℂ) ^ (ν - s) * (p.2 : ℂ) ^ (-s)) := by
        have h_fubini : Summable (fun p : ℕ+ => (p : ℂ) ^ (ν - s)) ∧ Summable (fun p : ℕ+ => (p : ℂ) ^ (-s)) := by
          constructor;
          · have h_summable : Summable (fun p : ℕ => (p : ℂ) ^ (ν - s)) := by
              refine' .of_norm _;
              have h_summable : Summable (fun p : ℕ => (p : ℝ) ^ (-(s - ν).re)) := by
                exact Real.summable_nat_rpow.2 ( by linarith );
              convert h_summable using 1;
              ext; rw [ ← Complex.ofReal_natCast ] ; rw [ Complex.norm_cpow_eq_rpow_re_of_nonneg ] <;> norm_num ; ring;
      ...
      rw [ mul_assoc, Complex.cpow_def_of_ne_zero, Complex.cpow_def_of_ne_zero, Complex.cpow_def_of_ne_zero ] <;> norm_num ; ring;
      rw [ Complex.log_mul ( by norm_num ) ( by norm_num ) ] ; rw [ Complex.cpow_def_of_ne_zero ( by norm_num ) ] ; ring;
      · rw [ ← Complex.exp_add ] ; ring;
      · norm_num [ Complex.arg ];
        exact ⟨ Real.pi_pos, Real.pi_pos.le ⟩;
    have h_fubini : ∑' (n : ℕ+), (∑ d ∈ (n : ℕ).divisors, (d : ℂ) ^ ν) * (n : ℂ) ^ (-s) = ∑' (p : ℕ+ × ℕ+), (p.1 : ℂ) ^ ν * (p.1 * p.2 : ℂ) ^ (-s) := by
      have h_fubini : ∀ n : ℕ+, (∑ d ∈ (n : ℕ).divisors, (d : ℂ) ^ ν) * (n : ℂ) ^ (-s) = ∑' (p : ℕ+ × ℕ+), (if p.1 * p.2 = n then (p.1 : ℂ) ^ ν * (p.1 * p.2 : ℂ) ^ (-s) else 0) := by
        intro n
        have h_fubini : (∑ d ∈ (n : ℕ).divisors, (d : ℂ) ^ ν) * (n : ℂ) ^ (-s) = ∑' (p : ℕ+ × ℕ+), (if p.1 * p.2 = n then (p.1 : ℂ) ^ ν * (n : ℂ) ^ (-s) else 0) := by
          rw [ tsum_eq_sum ];
          any_goals exact Finset.filter ( fun p => p.1 * p.2 = n ) ( Finset.Iic ( n, n ) );
          · simp +decide [ Finset.sum_ite, Finset.sum_mul _ _ _ ];
            refine' Finset.sum_bij ( fun x hx => ( ⟨ x, Nat.pos_of_mem_divisors hx ⟩, ⟨ n / x, Nat.div_pos ( Nat.le_of_dvd n.pos ( Nat.dvd_of_mem_divisors hx ) ) ( Nat.pos_of_mem_divisors hx ) ⟩ ) ) _ _ _ _ <;> simp +decide [ Nat.div_mul_cancel ( Nat.dvd_of_mem_divisors _ ) ];
            · exact fun a ha => ⟨ ⟨ Nat.le_of_dvd n.pos ha, Nat.div_le_self _ _ ⟩, PNat.eq <| Nat.mul_div_cancel' ha ⟩;
      ...
      · rw [ summable_iff_vanishing ] at *;
        intro e he; rcases ‹∀ e ∈ nhds 0, ∃ s_1 : Finset ( ℕ+ × ℕ+ ), ∀ t : Finset ( ℕ+ × ℕ+ ), Disjoint t s_1 → ∑ b ∈ t, ( b.1 : ℂ ) ^ ν * ( b.1 * b.2 : ℂ ) ^ ( -s ) ∈ e› e he with ⟨ s, hs ⟩ ; use s.image fun p => ( p.1 * p.2, p.1, p.2 ) ; intro t ht; simp_all +decide [ Finset.disjoint_left ] ;
        convert hs ( Finset.image ( fun p : ℕ+ × ℕ+ × ℕ+ => ( p.2.1, p.2.2 ) ) ( t.filter fun p : ℕ+ × ℕ+ × ℕ+ => p.1 = p.2.1 * p.2.2 ) ) _ using 1;
        · rw [ Finset.sum_image ];
  ...
  · convert h_lseries_def ( sigmaR ν ) s hs using 1;
    ...
    · aesop_cat;
    · intro x hx hx' hx''; use ⟨ x, Nat.pos_of_ne_zero hx ⟩ ; aesop;
  · have h_inner : ∀ d : ℕ+, ∑' (m : ℕ+), (d * m : ℂ) ^ (-s) = (d : ℂ) ^ (-s) * riemannZeta s := by
      intro d; rw [ show riemannZeta s = ∑' m : ℕ+, ( m : ℂ ) ^ ( -s ) from ?_ ] ; rw [ ← tsum_mul_left ] ; congr; ext m; rw [ Complex.cpow_def_of_ne_zero, Complex.cpow_def_of_ne_zero ] <;> norm_num ; ring;
      · rw [ Complex.log_mul ( by norm_num ) ( by norm_num ) ] ; rw [ Complex.cpow_def_of_ne_zero ( by norm_num ) ] ; ring;
