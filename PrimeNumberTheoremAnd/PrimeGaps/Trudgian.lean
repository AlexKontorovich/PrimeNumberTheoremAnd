...
abbrev X₀ : ℕ := 2898243
@[simp] lemma X₀_eq : X₀ = 2898243 := rfl

noncomputable def δ (x : ℝ) : ℝ := 1 / (111 * (Real.log x) ^ (2 : ℝ))
@[simp] lemma δ_def (x : ℝ) : δ x = 1 / (111 * (Real.log x) ^ (2 : ℝ)) := rfl
...
lemma δ_nonneg {x : ℝ} (hx : (X₀ : ℝ) ≤ x) : 0 ≤ δ x := by
  ...
  have hpow : 0 ≤ (Real.log x) ^ (2 : ℝ) := by
    exact Real.rpow_nonneg hlog _
  have hden : 0 ≤ (111 : ℝ) * (Real.log x) ^ (2 : ℝ) := by
    exact mul_nonneg (by norm_num : (0 : ℝ) ≤ (111 : ℝ)) hpow
  have hδ : 0 ≤ (1 : ℝ) / ((111 : ℝ) * (Real.log x) ^ (2 : ℝ)) := by
    exact div_nonneg (by exact zero_le_one) hden
  simpa [δ] using hδ
...
lemma δ_strictly_decreasing {x y : ℝ}
  ...
  have hpow_lt : (Real.log x) ^ (2 : ℝ) < (Real.log y) ^ (2 : ℝ) := by
    exact Real.rpow_lt_rpow hlogx_pos.le hlog_lt (by norm_num)
  have hden_lt : (111 : ℝ) * (Real.log x) ^ (2 : ℝ) < (111 : ℝ) * (Real.log y) ^ (2 : ℝ) := by
    have h111_pos : (0 : ℝ) < (111 : ℝ) := by norm_num
    exact mul_lt_mul_of_pos_left hpow_lt h111_pos
  have hden_pos : 0 < (111 : ℝ) * (Real.log x) ^ (2 : ℝ) := by
    have hpow_pos : 0 < (Real.log x) ^ (2 : ℝ) :=
      Real.rpow_pos_of_pos hlogx_pos _
    nlinarith

  -- `a < b` with `0 < a` gives `1/b < 1/a`.
  have hdiv : (1 : ℝ) / ((111 : ℝ) * (Real.log y) ^ (2 : ℝ))
      < (1 : ℝ) / ((111 : ℝ) * (Real.log x) ^ (2 : ℝ)) := by
    simpa [one_div] using (one_div_lt_one_div_of_lt hden_pos hden_lt)
  simpa [δ] using hdiv
...
lemma delta_sixth_power_lt_sqrt {n : ℕ} (hn : n ≥ X₀ ^ 2) :
  ...
  have hδ_lt1 : δ (√(n : ℝ)) < 1 := by
    ...
    have hpow_gt1 : (1 : ℝ) < (Real.log (√(n : ℝ))) ^ (2 : ℝ) := by
      have : (1 : ℝ) ^ (2 : ℝ) < (Real.log (√(n : ℝ))) ^ (2 : ℝ) :=
        Real.rpow_lt_rpow hone_nonneg hlog_gt1 h2pos
      simpa using this
    have hden_gt1 : (1 : ℝ) < (111 : ℝ) * (Real.log (√(n : ℝ))) ^ (2 : ℝ) := by
      nlinarith
    have hone_pos : (0 : ℝ) < 1 := by norm_num
    have : (1 : ℝ) / ((111 : ℝ) * (Real.log (√(n : ℝ))) ^ (2 : ℝ)) < (1 : ℝ) / 1 := by
      simpa [one_div] using (one_div_lt_one_div_of_lt hone_pos hden_gt1)
    simpa [δ] using this
...
lemma eps_log_bound {n : ℕ} (hn : n ≥ X₀ ^ 2) :
  ...
  have hδX0_le : δ (X₀ : ℝ) ≤ (0.000675 : ℝ) := by
    -- Rewrite δ in terms of a natural power, then bound using `11.4 < log X₀`.
    have h11_pos : (0 : ℝ) < (11.4 : ℝ) := by norm_num
    have h11_pow_pos : (0 : ℝ) < (11.4 : ℝ) ^ (2 : ℕ) := by
      exact pow_pos h11_pos _
    have hpow_lt : (11.4 : ℝ) ^ (2 : ℕ) < (Real.log (X₀ : ℝ)) ^ (2 : ℕ) := by
      have h11_nonneg : (0 : ℝ) ≤ (11.4 : ℝ) := by norm_num
      exact pow_lt_pow_left₀ hlog_X0_gt h11_nonneg (n := 2) (by decide)
    have hden_lt : (111 : ℝ) * (11.4 : ℝ) ^ (2 : ℕ) < (111 : ℝ) * (Real.log (X₀ : ℝ)) ^ (2 : ℕ) := by
      have h111_pos : (0 : ℝ) < (111 : ℝ) := by norm_num
      exact mul_lt_mul_of_pos_left hpow_lt h111_pos
    have hden_pos : 0 < (111 : ℝ) * (11.4 : ℝ) ^ (2 : ℕ) := by
      nlinarith
    have hone_div_lt : (1 : ℝ) / ((111 : ℝ) * (Real.log (X₀ : ℝ)) ^ (2 : ℕ))
        < (1 : ℝ) / ((111 : ℝ) * (11.4 : ℝ) ^ (2 : ℕ)) := by
      simpa [one_div] using (one_div_lt_one_div_of_lt hden_pos hden_lt)
    have hone_div_le : (1 : ℝ) / ((111 : ℝ) * (11.4 : ℝ) ^ (2 : ℕ)) ≤ (0.000675 : ℝ) := by
      norm_num
    have : (1 : ℝ) / ((111 : ℝ) * (Real.log (X₀ : ℝ)) ^ (2 : ℕ)) ≤ (0.000675 : ℝ) :=
...
theorem prime_in_Icc {x : ℝ} (hx : (X₀ : ℝ) ≤ x) :
  ...
  have hx' : x > (2898242 : ℝ) := by
    have h : (2898242 : ℝ) < (X₀ : ℝ) := by
      norm_num [X₀]
    exact lt_of_lt_of_le h hx

  rcases Trudgian2016.has_prime_in_interval x hx' with ⟨p, hp, hxp, hple⟩
  refine ⟨p, hp, ?_, ?_⟩
  · simpa using hxp
  · have hple' : (p : ℝ) ≤ x + x / (111 * (Real.log x) ^ (2 : ℕ)) := by
      simpa using hple
    -- Work in the simp normal form: `δ x = (Real.log x ^ 2)⁻¹ * 111⁻¹`.
    set a : ℝ := (Real.log x ^ (2 : ℕ))⁻¹ * (111 : ℝ)⁻¹
    have hple'' : (p : ℝ) ≤ x + x * a := by
      simpa [a, div_eq_mul_inv, mul_assoc, mul_left_comm, mul_comm] using hple'
    have hx_mul : x * (1 + a) = x + x * a := by ring
    have hp_le : (p : ℝ) ≤ x * (1 + a) := by
      simpa [hx_mul] using hple''
    -- Now rewrite back to `δ`.
    simpa [a, δ, Real.rpow_natCast, mul_assoc, mul_left_comm, mul_comm] using hp_le
