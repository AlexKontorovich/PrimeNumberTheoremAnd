...
namespace PrimeGaps
namespace Dusart_old

abbrev X₀ : ℕ := 468991632
@[simp] lemma X₀_eq : X₀ = 468991632 := rfl

noncomputable def δ (x : ℝ) : ℝ := 1 / (5000 * (Real.log x) ^ (2 : ℝ))
@[simp] lemma δ_def (x : ℝ) : δ x = 1 / (5000 * (Real.log x) ^ (2 : ℝ)) := rfl
...
lemma δ_nonneg {x : ℝ} (hx : (X₀ : ℝ) ≤ x) : 0 ≤ δ x := by
  ...
  have hden : 0 ≤ (5000 : ℝ) * (Real.log x) ^ (2 : ℝ) := by
    exact mul_nonneg (by norm_num : (0 : ℝ) ≤ (5000 : ℝ)) hpow
  have hδ : 0 ≤ (1 : ℝ) / ((5000 : ℝ) * (Real.log x) ^ (2 : ℝ)) := by
...
lemma δ_strictly_decreasing {x y : ℝ}
  ...
  have hden_lt : (5000 : ℝ) * (Real.log x) ^ (2 : ℝ) < (5000 : ℝ) * (Real.log y) ^ (2 : ℝ) := by
    have h111_pos : (0 : ℝ) < (5000 : ℝ) := by norm_num
    exact mul_lt_mul_of_pos_left hpow_lt h111_pos
  have hden_pos : 0 < (5000 : ℝ) * (Real.log x) ^ (2 : ℝ) := by
    have hpow_pos : 0 < (Real.log x) ^ (2 : ℝ) :=
      Real.rpow_pos_of_pos hlogx_pos _
    nlinarith

  -- `a < b` with `0 < a` gives `1/b < 1/a`.
  have hdiv : (1 : ℝ) / ((5000 : ℝ) * (Real.log y) ^ (2 : ℝ))
      < (1 : ℝ) / ((5000 : ℝ) * (Real.log x) ^ (2 : ℝ)) := by
...
lemma delta_sixth_power_lt_sqrt {n : ℕ} (hn : n ≥ X₀ ^ 2) :
  ...
  have hδ_lt1 : δ (√(n : ℝ)) < 1 := by
    ...
    have hpow_gt1 : (1 : ℝ) < (Real.log (√(n : ℝ))) ^ (2 : ℝ) := by
      ...
      simpa using this
    have hden_gt1 : (1 : ℝ) < (5000 : ℝ) * (Real.log (√(n : ℝ))) ^ (2 : ℝ) := by
      nlinarith
    have hone_pos : (0 : ℝ) < 1 := by norm_num
    have : (1 : ℝ) / ((5000 : ℝ) * (Real.log (√(n : ℝ))) ^ (2 : ℝ)) < (1 : ℝ) / 1 := by
...
lemma eps_log_bound {n : ℕ} (hn : n ≥ X₀ ^ 2) :
  ...
  have hδX0_le : δ (X₀ : ℝ) ≤ (0.000675 : ℝ) := by
    ...
    have hden_lt : (5000 : ℝ) * (11.4 : ℝ) ^ (2 : ℕ) < (5000 : ℝ) * (Real.log (X₀ : ℝ)) ^ (2 : ℕ) := by
      have h111_pos : (0 : ℝ) < (5000 : ℝ) := by norm_num
      exact mul_lt_mul_of_pos_left hpow_lt h111_pos
    have hden_pos : 0 < (5000 : ℝ) * (11.4 : ℝ) ^ (2 : ℕ) := by
      nlinarith
    have hone_div_lt : (1 : ℝ) / ((5000 : ℝ) * (Real.log (X₀ : ℝ)) ^ (2 : ℕ))
        < (1 : ℝ) / ((5000 : ℝ) * (11.4 : ℝ) ^ (2 : ℕ)) := by
      simpa [one_div] using (one_div_lt_one_div_of_lt hden_pos hden_lt)
    have hone_div_le : (1 : ℝ) / ((5000 : ℝ) * (11.4 : ℝ) ^ (2 : ℕ)) ≤ (0.000675 : ℝ) := by
      norm_num
    have : (1 : ℝ) / ((5000 : ℝ) * (Real.log (X₀ : ℝ)) ^ (2 : ℕ)) ≤ (0.000675 : ℝ) :=
...
theorem prime_in_Icc {x : ℝ} (hx : (X₀ : ℝ) ≤ x) :
  ...
  have hx' : x ≥ 468991632 := by
    simpa [X₀] using hx
  rcases Dusart.corollary_5_5 (x := x) hx' with ⟨p, hp, hxp, hple⟩
  refine ⟨p, hp, ?_, ?_⟩
  · simpa using hxp
  · -- `HasPrimeInInterval x h` gives the additive bound `p ≤ x + h`.
    have hple' : (p : ℝ) ≤ x + x / (5000 * (Real.log x) ^ 2) := by
      simpa using hple

    -- Rewrite the endpoint into the multiplicative form `x * (1 + δ x)`.
    have hx_endpoint : x + x / (5000 * (Real.log x) ^ 2) = x * (1 + δ x) := by
      -- Now rewrite the RHS into the LHS form.
      calc
        x + x / (5000 * (Real.log x) ^ 2)
            = x + x * (1 / (5000 * (Real.log x) ^ 2)) := by
                simp [div_eq_mul_inv]
        _   = x + x * δ x := by simp [δ, div_eq_mul_inv]
        _   = x * (1 + δ x) := by ring
