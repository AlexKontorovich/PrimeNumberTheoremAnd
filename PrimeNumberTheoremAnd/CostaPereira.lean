  ...
  have theta_zero_large_k :
    ...
    calc x ^ (1 / ((k.succ : ℕ) : ℝ))
      ...
      _ < (2 ^ (k.succ : ℕ)) ^ (((k.succ : ℕ) : ℝ)⁻¹) :=
          rpow_lt_rpow hx.le this <| inv_pos.mpr <| cast_pos.mpr k.succ_pos
      _ = 2 := by
        rw [← rpow_natCast, ← rpow_mul zero_le_two, mul_inv_cancel₀, rpow_one]
        positivity
  rw [tsum_eq_sum (s := Iio ⌊log x / Real.log 2⌋₊)
    (fun k hk ↦ theta_zero_large_k k (not_lt.mp (mem_Iio.not.mp hk)))]
  by_cases hx1 : x < 1
  ...
  have h_eq :
    ...
    refine sum_bij' (fun k _ ↦ k.succ) (fun k _ ↦ k.pred)
      ...
      (fun k hk ↦ by
        ...
        exact mem_Iio.mpr (pred_lt_pred (one_le_iff_ne_zero.mp hk1) (lt_succ_of_le hk2)))
...
theorem sublemma_1_2 {x : ℝ} (hx : 0 < x) (n : ℝ) :
  ...
  simp_rw [sublemma_1_1 (rpow_pos_of_pos hx _), ← rpow_mul (le_of_lt hx),
...
theorem sublemma_1_3 {x : ℝ} (hx : 0 < x) :
  ...
  have h_summable : Summable (fun k : ℕ ↦ θ (x ^ (1 / (k.succ : ℝ)))) := by
    obtain ⟨N, hN⟩ : ∃ N : ℕ, ∀ k ≥ N, θ (x ^ (1 / (k.succ : ℝ))) = 0 := by
      ...
      norm_num
      rw [← log_lt_log_iff (rpow_pos_of_pos hx _) zero_lt_two, log_rpow hx,
    ...
    exact summable_of_ne_finset_zero fun k hk ↦
      hN k <| le_of_not_gt fun hk' ↦ hk <| mem_range.mpr hk'
  rw [← h_summable.sum_add_tsum_nat_add 1, ← tsum_even_add_odd] <;>
    norm_num [add_assoc, mul_add]
  · convert h_summable.comp_injective
      (show (fun k ↦ 2 * k + 1).Injective from fun a b h ↦ by grind) using 2
    grind
  · convert h_summable.comp_injective
      (show (fun k ↦ 2 * k + 2).Injective from fun a b h ↦ by grind) using 2
    grind
  ...
  have h_split_sum :
    ...
    have h_split :
      ...
      have hsum :
        ...
        simp only [this,
          sum_insert (by decide : (0 : ZMod 3) ∉ ({1, 2} : Finset _)),
          sum_insert (by decide : (1 : ZMod 3) ∉ ({2} : Finset _)),
          sum_singleton,
    ...
    · norm_num [Nat.succ_eq_add_one, mul_add]
      ring_nf
    · obtain ⟨N, hN⟩ : ∃ N : ℕ, ∀ k ≥ N,
        θ (x ^ (1 / (2 * (k.succ : ℝ) + 1))) = 0 := by
        obtain ⟨N, hN⟩ :
            ∃ N : ℕ, ∀ k ≥ N, x ^ (1 / (2 * (k.succ : ℝ) + 1) : ℝ) < 2 := by
          have h_tends_one :
              Tendsto (fun k : ℕ ↦ x ^ (1 / (2 * (k.succ : ℝ) + 1) : ℝ))
                atTop (nhds 1) := by
      ...
      exact summable_nat_add_iff N |>.1 <|
        ⟨_, hasSum_single 0 fun k hk ↦ hN _ <| Nat.le_add_left ..⟩
...
theorem sublemma_1_5 {x : ℝ} (hx : 0 < x) :
  ...
  have summable_of_eventually_zero {f : ℕ → ℝ} (hf : ∀ᶠ n in atTop, f n = 0) :
      Summable f := by
  ...
  have summable_even {x : ℝ} {hx : 0 < x} :
      Summable (fun k : ℕ ↦ θ (x ^ (1 / (3 * (2 * k : ℝ))))) :=
  ...
  have step1 :
      ∑' k : ℕ, θ (x ^ (1 / (3 * k.succ) : ℝ)) =
        ∑' n : ℕ, θ (x ^ (1 / (3 * n) : ℝ)) := by
    rw [Summable.tsum_eq_zero_add this, cast_zero, mul_zero, div_zero, rpow_zero,
...
theorem sublemma_1_6 {x : ℝ} (hx : 0 < x) :
    ψ x - θ x =
      ...
      ∑' (k : ℕ),
        θ (x ^ (1 / (6 * ((k.succ : ℕ) : ℝ) + 1))) := by
  rw [sublemma_1_4 hx, sublemma_1_5 hx]
  ring
...
theorem sublemma_1_7 {x : ℝ} (hx : 0 < x) :
  ...
  have h_split :
    ...
    · have h_theta_zero : ∀ᶠ k : ℕ in atTop, θ (x ^ (1 / (6 * (k + 1) + 1 : ℝ))) = 0 := by
        have h_theta_zero :
            Tendsto (fun k : ℕ ↦ x ^ (1 / (6 * (k + 1) + 1 : ℝ))) atTop (nhds 1) := by
          simpa using tendsto_const_nhds.rpow
            (tendsto_inv_atTop_zero.comp <|
              tendsto_atTop_mono (fun k ↦ by linarith) tendsto_natCast_atTop_atTop)
            (by simp only [ne_eq, hx.ne', not_false_eq_true, Or.inl])
      ...
      exact ⟨_, hasSum_single 0 fun n hn ↦ by simpa using hN (n + N) (by omega)⟩
  ...
  · refine summable_of_ne_finset_zero (s := range (floor (logb 2 x) + 1)) ?_
    simp only [mem_range, Order.lt_add_one_iff, not_le, succ_eq_add_one, cast_add,
      cast_one, one_div]
    intro k hk
    rw [theta_eq_zero_of_lt_two]
    rw [← log_lt_log_iff (rpow_pos_of_pos hx ..) zero_lt_two, log_rpow hx,
      inv_mul_lt_iff₀ (by linarith)]
    have := Nat.lt_of_floor_lt hk
    rw [logb, div_lt_iff₀ (log_pos one_lt_two)] at this
    nlinarith [Real.log_pos one_lt_two]
  · refine summable_of_ne_finset_zero
      (s := range (Nat.floor (log x / Real.log 5) + 1)) fun k hk ↦ ?_
    rw [theta_eq_zero_of_lt_two]
    norm_num
    rw [← log_lt_log_iff (by positivity) (by positivity), log_rpow hx, inv_mul_eq_div,
      div_mul_eq_mul_div, div_lt_iff₀] <;> norm_num at *
    · have := Nat.lt_of_floor_lt hk
