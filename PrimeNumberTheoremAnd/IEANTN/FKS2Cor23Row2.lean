...
/-- Quarter (`B=1/4`) Buthe floor assembler: the SQUARED slab check
`lhsE² ≤ rhsE2` (integer-power, rational) plus the squared-curve domination
`eval rhsE2 ≤ (rowcurve)²` give `Eπ ≤ rowcurve` on `[e^xlo, e^10]`, by taking
square roots outside the kernel.  Companion of `floor_buthe_of_curve_gen` for the
half-power rows. -/
theorem floor_buthe_quarter_of_curve (rhsE2 : Expr) (A C : ℝ) (xlo : ℝ) (slabLo : ℚ) (n : ℕ)
  ...
  -- L² ≤ eval rhsE2 ≤ (admissible)²  ⟹  L ≤ admissible
