...
private lemma residue_toMeromorphicNFOn_eq_residue
  ...
  exact residue_eq_of_tendsto
    ((by simpa [residue] using tendsto_nhds_limUnder h_exists).congr'
      (by filter_upwards [hf_mero.toMeromorphicNFOn_eq_self_on_nhdsNE hpU] with z hz
          simp [hz]).symm)
...
private lemma sub_principalPart_analyticAt_of_not_mem_poles
  ...
  exact (h_nf.meromorphicOrderAt_nonneg_iff_analyticAt.1 hxneg).sub
    (Finset.analyticAt_fun_sum polesFin fun p hp =>
      by fun_prop (disch := by rintro rfl; exact hxnp hp))
...
private lemma meromorphicOrderAt_sub_principalPart_nonneg
  ...
  have hcore : (f - fun z ↦ residue f p / (z - p)) =O[nhdsWithin p ({p}ᶜ)] (1 : ℂ → ℂ) :=
    simplePole_sub_residue_isBigO_one h_mero h_ord
  ...
  have hrest_cont : ContinuousAt rest p := by
    ...
    fun_prop (disch := exact sub_ne_zero.mpr (Finset.mem_erase.mp hq).1.symm)
  have hrest : rest =O[nhdsWithin p ({p}ᶜ)] (1 : ℂ → ℂ) :=
    IsBigO.mono hrest_cont.norm.isBoundedUnder_le.isBigO_one ℂ inf_le_left
...
private lemma holoPart_holomorphicOn {f : ℂ → ℂ} {z w : ℂ}
  ...
  have h_raw_mero : MeromorphicOn (fNF - principalPart) R := h_fNF_mero.sub principalPart_meromorphicOn
  intro x hx
  have h_raw_nonneg : 0 ≤ meromorphicOrderAt (fNF - principalPart) x := by
    ...
    · have hpFin : x ∈ polesFin := by simpa [polesFin, poles] using hxp
      have hordNF : meromorphicOrderAt fNF x = (-1 : ℤ) := by
        rwa [show meromorphicOrderAt fNF x = meromorphicOrderAt f x from by
          simpa [fNF] using
    ...
    · have h_fNF_nonneg : 0 ≤ meromorphicOrderAt fNF x := by
      ...
      exact (sub_principalPart_analyticAt_of_not_mem_poles
        (by simpa [fNF] using meromorphicNFOn_toMeromorphicNFOn (f := f) (U := R) hx)
        (fun h => hxp (by simpa [polesFin, poles] using h))
        h_fNF_nonneg).meromorphicOrderAt_nonneg
...
private lemma principalPart_borderIntegrable {f : ℂ → ℂ} {z w : ℂ}
  ...
  exact (by fun_prop (disch := exact sub_ne_zero.mpr hsp)).continuousWithinAt
...
private lemma continuousOn_toMeromorphicNFOn_upperRboundary {F : ℂ → ℂ} (l : LadderParams)
  ...
  have h_order : 0 ≤ meromorphicOrderAt (toMeromorphicNFOn F l.R) s := by
    rw [meromorphicOrderAt_toMeromorphicNFOn hF_mero (LadderParams.Rboundary_subset_R l hs.1)]
    exact hF_nopoles s hs.1 hs.2
  have h_anal := h_NF_mero.meromorphicOrderAt_nonneg_iff_analyticAt.1 h_order
  exact h_anal.continuousAt.continuousWithinAt
...
private lemma G_mul_cpow_integrable_vseg_generic (l : LadderParams)
  ...
  have h_sum_cont : ContinuousOn (fun t : ℝ ↦ toMeromorphicNFOn (G_circ + G_star) l.R (1 + t * Complex.I)) (Set.Icc a b) :=
    ContinuousOn.comp h_cont_sum_NF h_path h_maps_rb
...
theorem lemma_5_1_a (n : ℕ)
  ...
  have hG_nopoles : ∀ s ∈ l.Rboundary, 0 ≤ s.im → 0 ≤ meromorphicOrderAt (G_circ + G_star) s := by
    ...
    · -- Case 1: s.im > 0, so G = G_circ + G_star near s, and G has no poles on Rboundary (hG_bdd)
      ...
      have hG_mero : MeromorphicAt G s := by
        have h_sum_mero : MeromorphicAt (G_circ + G_star) s :=
          (hG_circ_mero s (l.Rboundary_subset_R hs)).add (hG_star_mero s (l.Rboundary_subset_R hs))
        exact h_sum_mero.congr (hG_eq.symm.filter_mono nhdsWithin_le_nhds)
    ...
    · -- Case 2: s.im = 0, so s = 1. G_circ and G_star have no poles at 1 from hGc_contour and hGs_contour since 1 ∈ admissible_contour
      ...
      have hs_eq : s = 1 := by
        ...
        · simp [hs_re]
        · simp [hs_im_zero]
      have h1_contour : (1 : ℂ) ∈ l.admissible_contour := by
        simp only [LadderParams.admissible_contour, Set.mem_setOf_eq]
        right
        refine ⟨by simp, ?_⟩
        simp only [Set.mem_Icc, Complex.one_im]
        refine ⟨le_rfl, le_of_lt l.hδ.1⟩
  ...
  have h_unprimed_eq : intVSeg 1 0 l.T (fun s ↦ G s * (x : ℂ) ^ s) =
    ...
    RectangleIntegral (fun s ↦ G s * (x : ℂ) ^ s) ((l.σ n : ℂ) + (l.δ : ℂ) * Complex.I) (1 + (l.T : ℂ) * Complex.I) :=
      intVSeg_eq_intCnPlus_add_rectangleIntegral l n (fun s ↦ G s * (x : ℂ) ^ s)
        (G_mul_cpow_integrable_vseg_generic l hG hG_circ_mero hG_star_mero hx₀ hG_nopoles hx 0 l.δ (by rfl) (le_of_lt l.hδ.1) (by linarith [l.hδ.2, l.hT]))
        (G_mul_cpow_integrable_vseg_generic l hG hG_circ_mero hG_star_mero hx₀ hG_nopoles hx l.δ l.T (le_of_lt (by linarith [l.hδ.1])) (by linarith [l.hδ.2, l.hT]) le_rfl)
