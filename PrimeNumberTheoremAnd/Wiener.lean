...
@[blueprint
  ...
  (statement := /-- If $I$ is a closed interval contained in an open interval $J$, then there exists a smooth function $\Psi: \R \to \R$ with $1_I \leq \Psi \leq 1_J$. -/)
  (proof := /-- A standard analysis lemma, which can be proven by convolving $1_K$ with a smooth approximation to the identity for some interval $K$ between $I$ and $J$. Note that we have ``SmoothBumpFunction''s on smooth manifolds in Mathlib, so this shouldn't be too hard... -/)
  (latexEnv := "lemma")]
...
lemma auto_cheby_exists_smooth_nonneg_fourier_kernel :
  ...
  · -- Proof that \hat{\psi}(0) > 0
    ...
    have hFφ0_ne : (𝓕 φ 0) ≠ 0 := by
        have hre : 0 < (𝓕 φ 0).re := by
          ...
          have : (∫ x, Complex.ofReal (φ_real x)).re = ∫ x, φ_real x := by
            ...
            have h := integral_re (μ := volume) h_int_complex
            simpa using h.symm
          rw [this]
          exact hφint_pos
    ...
    rw [h_conj, mul_comm, ← Complex.normSq_eq_conj_mul_self]
    exact Complex.normSq_pos.2 hFφ0_ne
...
over a short interval $((1-\epsilon)x, x]$ is bounded by $O(x)$.
-/
