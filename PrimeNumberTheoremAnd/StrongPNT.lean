...
@[blueprint "PathIntegral"
  (title := "PathIntegral")
  (statement := /-- Let $0 < r < R<1$. Let $f:\overline{\mathbb{D}_R}\to\mathbb{C}$ be analytic on neighborhoods of points in $\overline{\mathbb{D}_R}$. Define the functon $I_f:\overline{\mathbb{D}_r}\to\mathbb{C}$ by
...
@[blueprint "LogOfAnalyticFunction"
  ...
  (proof := /-- We let $J_B(z)=I_{B'/B}(z)$. Then clearly, $J_B(0)=0$. Now note that
    ...
    $$\frac{1}{B(0)}=\frac{\exp(J_B(z))}{B(z)}\implies\abs{\frac{B(z)}{B(0)}}=\exp(\mathfrak{R}J_B(z)).$$
