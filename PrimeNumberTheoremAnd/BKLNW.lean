...
@[blueprint
  "bklnw-cor-2-1"
  (title := "Corollary 2.1")
  (statement := /-- $\theta(x) ≤ (1 + 1.93378 \times 10^{-8}) x$. -/)
  (latexEnv := "corollary")]
...
noncomputable def a₁ (b : ℝ) : ℝ :=
  if b ≤ 38 * log 10 then 1 + 1.93378e-8 else 1 + ε (b / 2)

noncomputable def a₂ (b : ℝ) : ℝ :=
  (1 + 1.93378e-8) * (max (f (exp b)) (f (⌊b / (log 2)⌋ + 1)))
