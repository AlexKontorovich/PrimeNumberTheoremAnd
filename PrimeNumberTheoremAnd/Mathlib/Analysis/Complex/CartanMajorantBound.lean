...
theorem cartan_sum_majorant_le
    ...
    (hr_phi :
      ...
      (∑ i ∈ small, (1 : ℝ) * LogSingularity.φ (r / a i))
        ≤ LogSingularity.Cφ * (∑ i ∈ small, (1 : ℝ))) :
    let small : Finset (divisorZeroIndex₀ f (Set.univ : Set ℂ)) := hsmall_fin.toFinset
  ...
  -- convert `hr_phi` to `∑_{p∈small} φ ≤ Cφ * card(small)`
  have hphi_sum : (∑ p ∈ small, LogSingularity.φ (r / ap p)) ≤ LogSingularity.Cφ * (small.card : ℝ) := by
    have h := hr_phi
    simpa [small, ap, Finset.sum_const, nsmul_eq_mul, mul_assoc, mul_left_comm, mul_comm] using h

  -- Nonnegativity and summability of `b`.
  ...
  have hb_summable : Summable b := by
    ...
    have hb₂ : Summable b₂ := by
      ...
      refine Summable.of_nonneg_of_le
        (fun p => by by_cases hp : p ∈ small <;> simp [b₂, hp] <;> positivity)
        (fun p => ?_) hconst
      by_cases hp : p ∈ small
  ...
  -- Sharp cardinal bound on `small` (Tao): `card(small) ≤ (4R)^τ * (Sτ+1)`.
  have hcard_le : (small.card : ℝ) ≤ (4 * R) ^ τ * (Sτ + 1) := by
    have hsum_le : (∑ p ∈ small, ‖divisorZeroIndex₀_val p‖⁻¹ ^ τ) ≤ Sτ := by
      have hnn :
          ∀ p : divisorZeroIndex₀ f (Set.univ : Set ℂ), 0 ≤ ‖divisorZeroIndex₀_val p‖⁻¹ ^ τ := by
        intro p; exact Real.rpow_nonneg (inv_nonneg.2 (norm_nonneg _)) _
      simpa [Sτ] using
        (Summable.sum_le_tsum (s := small)
          (f := fun p : divisorZeroIndex₀ f (Set.univ : Set ℂ) => ‖divisorZeroIndex₀_val p‖⁻¹ ^ τ)
          (fun p _ => hnn p) hsumτ)
    have hgeom_sum :
      ...
      have hp_le : ‖divisorZeroIndex₀_val p‖ ≤ 4 * R := by
        have : p ∈ smallSet := (hsmall_mem p).1 hp
        simpa [hsmallSet] using this
      have hap : 0 < ‖divisorZeroIndex₀_val p‖ := norm_pos_iff.2 (divisorZeroIndex₀_val_ne_zero p)
      ...
      have hdiv :
        ...
        simpa [div_eq_mul_inv, mul_assoc, mul_left_comm, mul_comm] using
          (Real.mul_rpow (x := (4 * R : ℝ)) (y := (‖divisorZeroIndex₀_val p‖ : ℝ)⁻¹) (z := τ) h4 ha)
    ...
    have hS_le : (4 * R) ^ τ * Sτ ≤ (4 * R) ^ τ * (Sτ + 1) := by
      ...
      linarith
    exact hsmall_le.trans hS_le

  -- Convert to the `4^τ * (1+r)^τ` form.
  have hR_le_1r : R ≤ 1 + r := by linarith [hR_le_r, le_of_lt hrpos]
  have hpowR : (4 * R) ^ τ ≤ (4 : ℝ) ^ τ * (1 + r) ^ τ := by
    ...
    have hmul : (4 * (1 + r)) ^ τ = (4 : ℝ) ^ τ * (1 + r) ^ τ := by
      ...
      simpa [mul_assoc] using (Real.mul_rpow (x := (4 : ℝ)) (y := (1 + r : ℝ)) (z := τ) h4 h1)
  ...
  -- Tao bookkeeping: bound `tsum b`.
  have hb_tsum_le :
      (∑' p : divisorZeroIndex₀ f (Set.univ : Set ℂ), b p) ≤ Cprod * (1 + r) ^ τ := by
    -- Majorize `b` by `bφ + b0 + bmτ + bt` and bound each `tsum`.
    ...
    have hb_pointwise : ∀ p, b p ≤ bφ p + b0 p + bmτ p + bt p := by
      ...
      · have hbase : 0 ≤ r / ap p := div_nonneg hr (by positivity)
        ...
        simp [b, bφ, b0, bmτ, bt, hp, add_assoc, add_left_comm, add_comm, mul_add,
          mul_assoc, mul_left_comm, mul_comm, hpos] <;> nlinarith
      ·
        have hbt_nonneg : 0 ≤ bt p := by
          have hbase : 0 ≤ r / ap p := div_nonneg hr (by positivity)
          have : 0 ≤ (r / ap p) ^ τ := Real.rpow_nonneg hbase τ
          simpa [bt] using mul_nonneg (by norm_num : (0 : ℝ) ≤ (2 : ℝ)) this
        simp [b, bφ, b0, bmτ, bt, hp, hbt_nonneg]
    ...
    have htsum_bφ :
        (∑' p : divisorZeroIndex₀ f (Set.univ : Set ℂ), bφ p) = ∑ p ∈ small, LogSingularity.φ (r / ap p) := by
      classical
      simpa [bφ] using
    ...
    have htsum_b0 :
      ...
      simp [this, Finset.sum_const, nsmul_eq_mul, mul_assoc, mul_left_comm, mul_comm]
    have htsum_bmτ :
        (∑' p : divisorZeroIndex₀ f (Set.univ : Set ℂ), bmτ p) = (m : ℝ) * ∑ p ∈ small, (r / ap p) ^ τ := by
      classical
      have : (∑' p : divisorZeroIndex₀ f (Set.univ : Set ℂ), bmτ p) = ∑ p ∈ small, (m : ℝ) * (r / ap p) ^ τ := by
        simpa [bmτ] using
    ...
    have htsum_bt :
      ...
      have hbt_eq :
        ...
        -- Avoid `simp` creating cancellation goals like `A * (r^τ) = B * (r^τ)`.
        -- We rewrite pointwise and pull constants out of the `tsum`.
        have hpow :
            ∀ p : divisorZeroIndex₀ f (Set.univ : Set ℂ),
              (r / ap p) ^ τ = (r ^ τ) * (‖divisorZeroIndex₀_val p‖⁻¹ : ℝ) ^ τ := by
          intro p
          have ha0 : 0 ≤ (‖divisorZeroIndex₀_val p‖⁻¹ : ℝ) := by positivity
          -- `ap p = ‖divisorZeroIndex₀_val p‖`
          simpa [ap, div_eq_mul_inv, mul_assoc, mul_left_comm, mul_comm] using
            (Real.mul_rpow (x := r) (y := (‖divisorZeroIndex₀_val p‖⁻¹ : ℝ)) (z := τ) hr ha0)
        calc
          (∑' p : divisorZeroIndex₀ f (Set.univ : Set ℂ), bt p)
              = (2 : ℝ) * ∑' p : divisorZeroIndex₀ f (Set.univ : Set ℂ), (r / ap p) ^ τ := by
                -- Pull the constant `2` out of the `tsum` without any side-goals.
          ...
          _ = (2 : ℝ) * ((r ^ τ) * ∑' p : divisorZeroIndex₀ f (Set.univ : Set ℂ),
                ...
                simp [tsum_mul_left, mul_assoc, mul_left_comm, mul_comm]
          _ = (2 : ℝ) * (r ^ τ) * Sτ := by
                simp [Sτ, mul_assoc, mul_left_comm, mul_comm]
      have h1r : r ^ τ ≤ (1 + r) ^ τ :=
    ...
    have hsum_small_rpow_le :
      ...
      have : ∑ p ∈ small, (r / ap p) ^ τ = (r ^ τ) * ∑ p ∈ small, (‖divisorZeroIndex₀_val p‖⁻¹ : ℝ) ^ τ := by
        classical
        simp [hrpow, Finset.mul_sum, mul_assoc, mul_left_comm, mul_comm]
      have h1r : r ^ τ ≤ (1 + r) ^ τ :=
        Real.rpow_le_rpow (by positivity) (by linarith) hτ_nonneg
    ...
    have htsum_majorant :
      ...
      have hbmτ_le :
        ...
        -- First use the sharp bound on the finite sum, then absorb it into a `4^τ`-multiple.
        have h0 :
            (∑' p : divisorZeroIndex₀ f (Set.univ : Set ℂ), bmτ p)
              ≤ (m : ℝ) * ((Sτ + 1) * (1 + r) ^ τ) := by
        ...
        have hscale :
          ...
          -- multiply the inequality `1 ≤ 4^τ` by the nonnegative factor `m * ((Sτ+1)*(1+r)^τ)`.
          have hnonneg : 0 ≤ (m : ℝ) * ((Sτ + 1) * (1 + r) ^ τ) := by
            have : 0 ≤ (Sτ + 1) * (1 + r) ^ τ := by positivity
            exact mul_nonneg hm0 this
          -- rewrite `x ≤ (4^τ) * x` as `1 * x ≤ (4^τ) * x`.
      ...
      have hsplit :
        ...
        -- We need summability of each term to split `tsum` over `+`.
        classical
        have hbφ_summ : Summable bφ := by
          refine summable_of_finite_support ?_
          refine (Finset.finite_toSet small).subset ?_
        ...
        -- Now split the big sum as `(bφ+b0) + (bmτ+bt)`.
        calc
          (∑' p : divisorZeroIndex₀ f (Set.univ : Set ℂ), (bφ p + b0 p + bmτ p + bt p))
          ...
          _ = (∑' p, bφ p) + (∑' p, b0 p) + (∑' p, bmτ p) + (∑' p, bt p) := by
              ring
      -- Bound the `tsum` by bounding each component and then rewriting via `hsplit`.
      have hcard_le'' :
          (small.card : ℝ) ≤ (4 : ℝ) ^ τ * (Sτ + 1) * (1 + r) ^ τ := by
        -- `hcard_le'` has the same factors but in a different order.
      ...
      have htsum_majorant' :
          (∑' p : divisorZeroIndex₀ f (Set.univ : Set ℂ), (bφ p + b0 p + bmτ p + bt p))
            ≤ (LogSingularity.Cφ + (2 : ℝ) * m) * (4 : ℝ) ^ τ * (Sτ + 1) * (1 + r) ^ τ
              + (2 : ℝ) * (Sτ + 1) * (1 + r) ^ τ := by
        -- rewrite the left-hand side using `hsplit`, then solve by linear arithmetic
        rw [hsplit]
        nlinarith [hφ_le', hb0_le', hbmτ_le, hbt_le, hS]
      have hring :
          (LogSingularity.Cφ + (2 : ℝ) * m) * (4 : ℝ) ^ τ * (Sτ + 1) * (1 + r) ^ τ
              + (2 : ℝ) * (Sτ + 1) * (1 + r) ^ τ
            = (((LogSingularity.Cφ + (2 : ℝ) * m) * (4 : ℝ) ^ τ + 2) * (Sτ + 1)) * (1 + r) ^ τ := by
        ring
      simpa [hring] using htsum_majorant'
