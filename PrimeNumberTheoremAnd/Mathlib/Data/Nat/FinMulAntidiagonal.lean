...
@[reducible]
private def f {n : ℕ} : ∀ (a : Fin 3 → ℕ) (_ : a ∈ finMulAntidiagonal 3 n), ℕ × ℕ := fun a _ =>
    (a 0 * a 1, a 0 * a 2)

private theorem finMulAntidiagonal_three {n : ℕ} : ∀ (a : Fin 3 → ℕ) (_ : a ∈ finMulAntidiagonal 3 n), a 0 * a 1 * a 2 = n := by
    intro a ha
...
private theorem f_img {n : ℕ} (hn : Squarefree n) : ∀ (a : Fin 3 → ℕ) (ha : a ∈ finMulAntidiagonal 3 n),
  ...
  intro a ha
  rw [mem_filter, Finset.mem_product, mem_divisors, mem_divisors, Nat.f]
  refine ⟨⟨⟨?_, hn.ne_zero⟩, ⟨?_, hn.ne_zero⟩⟩, ?_⟩ <;> rw [←finMulAntidiagonal_three a ha]
  · apply dvd_mul_right _ (a 2)
  · use a 1; ring
  dsimp only
...
private theorem f_inj {n : ℕ} (a : Fin 3 → ℕ) (ha : a ∈ finMulAntidiagonal 3 n)
    (b : Fin 3 → ℕ) (hb : b ∈ finMulAntidiagonal 3 n) (hfab : f a ha = f b hb) : a = b := by
...
theorem card_lcm_eq {n : ℕ} (hn : Squarefree n) :
  ...
  exact Finset.card_bij f (f_img hn) f_inj (f_surj hn.ne_zero)
